...
public class MinMaxDateInputValidator implements SmartValidator {
  ...
  //TODO add hints for min / max labels
  public static final String MIN_YEAR = "minYear";
  public static final String MAX_YEAR = "maxYear";
  public static final String DEFAULT_INPUT_LABEL_TEXT = "min and max years";
  public static final String ENTER_BOTH_YEARS_ERROR = "%s requires a minimum and maximum year";
  public static final String MIN_BEFORE_MAX_YEAR_ERROR = "%s's minimum year must be before the maximum year";
  public static final String MAX_YEAR_IN_FUTURE_ERROR = "%s's maximum year must be in the future";
  ...
  public void validate(Object target, Errors errors, Object... validationHints) {
    ...
    if (emptyMinMaxDateAcceptableHint.isPresent()) {
      if (minMaxDateInput.getMinYear() != null) {
        rejectIfInvalidYear(errors, MIN_YEAR, inputLabel, minMaxDateInput.getMinYear());
      }

      if (minMaxDateInput.getMaxYear() != null && isInvalidYear(minMaxDateInput.getMaxYear())) {
        rejectIfInvalidYear(errors, MAX_YEAR, inputLabel, minMaxDateInput.getMaxYear());
      }
      ...
      if (!bothDatesArePresent(minMaxDateInput)) {
        errors.rejectValue(
            ...
            getBothYearsRequiredErrorMessage(inputLabel)
        );
      }

      if (bothDatesArePresent(minMaxDateInput)) {
        //Check both years are valid - Only add the error once if
        rejectIfInvalidYear(errors, MIN_YEAR, inputLabel, minMaxDateInput.getMinYear());
        rejectIfInvalidYear(errors, MIN_YEAR, inputLabel, minMaxDateInput.getMaxYear());
      }
    }
    ...
    if (bothDatesArePresentAndValid(minMaxDateInput) && !minMaxDateInput.minIsBeforeMax()) {
      errors.rejectValue(
          MIN_YEAR,
          getInvalidErrorCode(MIN_YEAR),
          getMinYearBeforeMaxYearErrorMessage(inputLabel)
      );
    }
    ...
    if (maxYearMustBeInFutureHint.isPresent()
      ...
      errors.rejectValue(
          MIN_YEAR,
          getInvalidErrorCode(MIN_YEAR),
          getMaxYearMustBeInFutureErrorMessage(inputLabel)
      );
    }
  ...
  private boolean bothDatesArePresentAndValid(MinMaxDateInput minMaxDateInput) {
    return bothDatesArePresent(minMaxDateInput)
            &&
          (!isInvalidYear(minMaxDateInput.getMinYear()) && !isInvalidYear(minMaxDateInput.getMinYear()));
  }

  private String getBothYearsRequiredErrorMessage(FormInputLabel inputLabel) {
    return String.format(ENTER_BOTH_YEARS_ERROR, inputLabel.getInitCappedLabel());
  }

  private String getMinYearBeforeMaxYearErrorMessage(FormInputLabel inputLabel) {
    return String.format(MIN_BEFORE_MAX_YEAR_ERROR, inputLabel.getInitCappedLabel());
  }

  private String getMaxYearMustBeInFutureErrorMessage(FormInputLabel inputLabel) {
    return String.format(MAX_YEAR_IN_FUTURE_ERROR, inputLabel.getInitCappedLabel());
  }
  ...
  //TODO Add indicator for if min / max date
  private void rejectIfInvalidYear(Errors errors, String field, FormInputLabel inputLabel, String year) {
    if (isInvalidYear(year)) {
      errors.rejectValue(
          field,
          getInvalidErrorCode(field),
          DateInputValidator.getIncorrectYearFormatErrorMessage(inputLabel)
      );
    }
  }
