...
headerIcon=true>
  ...
  <header class="govuk-header<#if topNavigation> govuk-header--navigation-menu </#if>${headerClasses}" role="banner" data-module="govuk-header">
    <div class="govuk-header__container ${containerClasses}<#if wrapperWidth> govuk-width-container-wide<#else> govuk-width-container</#if>">
      ...
      <div class="govuk-header__logo">
        <a href="${homePageUrl}" class="govuk-header__link govuk-header__link--homepage">
          <span class="govuk-header__logotype">
            <#if headerIcon>
              ...
              <#else>
              <svg version="1.1" class="govuk-header__logotype-droplet" height="30" width="36" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 26.52 36" style="enable-background:new 0 0 26.52 36;" xml:space="preserve">
                <style type="text/css">.st5{fill:#FFFFFF;}</style>
                <path class="st5" d="M16.05,0C14.34,0,0,7.84,0,21.93C0,29.34,5.98,36,13.39,36c7.41,0,13.42-6.01,13.42-13.42c0-6.43-5.55-10.31-7.2-12.08C14.02,4.49,16.05,0,16.05,0z"/>
              </svg>
            </#if>
            <span class="govuk-header__logotype-text">${logoText}</span>
            <#if logoProductText?has_content>
              <span class="govuk-header__product-name">${logoProductText}</span>
            </#if>
          </span>
        </a>
      </div>
        ...
        <#if headerNav>
          <div class="govuk-header__content">
            <a href="${serviceUrl}" class="govuk-header__link govuk-header__link--service-name">${serviceName}</a>
